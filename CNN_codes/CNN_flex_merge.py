...
def CNNfm(input_x, input_y,input_variables,
          img_len=164, channel_num=8, output_size=2,
    ...
    combinedvec=tf.concat([flatten,input_variables],axis=1,name='combinedvec')
    comb_vector_length=combinedvec.shape[1]
    
    fc_w=[]
    
    
    fc_layer_0=fc_layer(input_x=combinedvec,in_size=comb_vector_length,out_size=fc_units[0],rand_seed=seed,activation_function=tf.nn.relu,index=0)
...
def cnnfm_training(X_train, y_train, X_trainvar,X_val, y_val, X_valvar,
             addedvar_num=500,
    ...
    with tf.name_scope('inputs'):
        ...
        xvar=tf.placeholder(shape=[None,addedvar_num],dtype=tf.float32)
        
    #xs,hmm=augment(xs,ys,horizontal_flip=True,rotate=3,crop_probability=0.4) 

    
    output,loss=CNNfm(xs,ys,xvar,img_len=imglen, channel_num=channum, output_size=2,conv_featmap=conv_featmap,fc_units=fc_units,conv_kernel_size=[3,3], pooling_size=[2,2],l2_norm=l2_norm, seed=seed)
    ...
    iter_total = 0
    best_acc = 0
    cur_model_name = 'cnnmf_{}'.format(int(time.time()))
    ...
    with tf.Session() as sess:
        ...
        for epc in range(epoch):
            ...
            for itr in range(iters):
                ...
                training_batch_xvar = X_trainvar[itr * batch_size: (1 + itr) * batch_size]
                ...
                if rng>60 & rng<=75:
                    ...
                    pass
                _, cur_loss = sess.run([step, loss], feed_dict={xs: training_batch_x, ys: training_batch_y,xvar:training_batch_xvar})

                if itr == iters-1:
                    # do validation
                    train_eve=sess.run(eve,feed_dict={xs:X_train,ys:y_train,xvar:X_trainvar})
                    train_acc=100-train_eve*100/y_train.shape[0]
                    store_trainacc.append(train_acc)
                    valid_eve, merge_result = sess.run([eve, merge], feed_dict={xs: X_val, ys: y_val,xvar:X_valvar})
                    valid_acc = 100 - valid_eve * 100 / y_val.shape[0]
                    store_valacc.append(valid_acc)
                    
                    #y=graph.get_tensor_by_name("evaluate/ArgMax:0")
                    
                    result=sess.run(pred,feed_dict={xs:X_val,ys:y_val,xvar:X_valvar})
                    percentage=sess.run(output,feed_dict={xs:X_val,ys: y_val,xvar:X_valvar})
    ...
    return best_acc,store_trainacc,store_valacc,store_pred,store_truelabel,percentage_out
