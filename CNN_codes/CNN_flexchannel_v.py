...
def CNN(input_x, input_y,
          img_len=13, channel_num=8, output_size=2,
          conv_featmap=[6, 16, 20], fc_units=[500,200,2],
    ...
    fc_layer_0=fc_layer(input_x=flatten,in_size=img_vector_length,out_size=fc_units[0],rand_seed=seed,activation_function=tf.nn.relu,index=0)
    ...
    fc_layer_2=fc_layer(input_x=fc_layer_1.output(),in_size=fc_units[1],out_size=fc_units[2],rand_seed=seed,activation_function=tf.nn.softmax,index=2)
    ...
    y_c=tf.reduce_sum(tf.multiply(fc_layer_2.output(),label),axis=1)
    gb_grad=tf.gradients(loss,input_x)[0]
    layer_grad=tf.gradients(y_c,pooling_layer_0.output())[0]
    return fc_layer_2.output(), loss, pooling_layer_0.output(),gb_grad,layer_grad
...
def cnn_training(X_train, y_train, X_val, y_val, 
    ...
    output,loss,conv_layer_0,gb_grad,layer_grad=CNN(xs,ys,img_len=imglen, channel_num=channum, output_size=2,conv_featmap=conv_featmap,fc_units=fc_units,conv_kernel_size=[3,3], pooling_size=[2,2],l2_norm=l2_norm, seed=seed)
    ...
    with tf.Session() as sess:
        ...
        writer = tf.summary.FileWriter("newlog/{}".format(cur_model_name), sess.graph)
        ...
        for epc in range(epoch):
            ...
            for itr in range(iters):
                ...
                if itr == iters-1:
                    ...
                    #writer.add_summary(conv_layer_0,iter_total)
                    ...
                    if valid_acc > best_acc:
                        ...
                        units=sess.run(conv_layer_0,feed_dict={xs:X_val})
                        grad_gb=sess.run(gb_grad,feed_dict={xs:X_val,ys:y_val})
                        grad_layer=sess.run(layer_grad,feed_dict={xs:X_val,ys:y_val})
                        val_pred=sess.run(pred,feed_dict={xs:X_val})
                        
                        units2=sess.run(conv_layer_0,feed_dict={xs:X_train})
                        grad_gb2=sess.run(gb_grad,feed_dict={xs:X_train,ys:y_train})
                        grad_layer2=sess.run(layer_grad,feed_dict={xs:X_train,ys:y_train})
                        truelabel_train=y_train
                        predlabel_train=sess.run(pred,feed_dict={xs:X_train})


    print("Traning ends. The best valid accuracy is {}. Model named {}.".format(best_acc, cur_model_name))
    
    return best_acc,store_trainacc,store_valacc,store_pred,store_truelabel,percentage_out,units,grad_gb,grad_layer,y_val,val_pred,units2,grad_gb2,grad_layer2,truelabel_train,predlabel_train
